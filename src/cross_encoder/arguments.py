...
class DataArguments:
    ...
    prediction_topk_min: int = field(
        default=0, metadata={"help": "max k"}
    )
    ...
    dev_neg_file: str = field(
        default=None, metadata={"help": "Path to train data"}
    )
    dev_corpus_file: str = field(
        default=None, metadata={"help": "Path to corpus"}
    )
    dev_orpus_id_file: str = field(
        default=None, metadata={"help": "Path to corpus"}
    )
    ...
    overwrite_prediction: bool = field(default=False, metadata={"help":"Whether to overwrite prediction"})
    ...
    def __post_init__(self):
        ...
        if self.dev_corpus_file and not self.dev_orpus_id_file:
            if not os.path.exists(os.path.join(self.corpus_file, 'mapping_id.txt')):
                raise FileNotFoundError(f'There is no mapping_id.txt in {self.dev_corpus_file}')
            self.dev_orpus_id_file = os.path.join(self.dev_corpus_file, 'mapping_id.txt')
        ...
        if self.dev_query_file and not self.dev_query_id_file:
            if not os.path.exists(os.path.join(self.dev_query_file, 'mapping_id.txt')):
                raise FileNotFoundError(f'There is no mapping_id.txt in {self.dev_query_file}')
            self.dev_query_id_file = os.path.join(self.dev_query_file, 'mapping_id.txt')
